...
class textrank_graph:
    def __init__(self):
        self.graph = defaultdict(list) # 创建一个字典，当所查询字典key值不存在时，返回一个空list
    ...
    def seg_files(self):
        ...
        for root, dirs, files in os.walk(self.trainfile):
            for file in files:
                ...
                print(filepath)
                # jieba分词
                word_list = [w.word for w in pseg.cut(open(filepath, encoding='gbk', errors='ignore').read()) \
                             if w.flag[0] not in ['x', 'w', 'u', 'c', 'p', 'q', 't', 'f', 'd'] and len(w.word) > 1]
                # 生成词频字典
